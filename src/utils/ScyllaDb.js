...
export default class ScyllaDb {
  ...
  /**
   * Load table configurations from a JS object (for tests/integration)
   */
  static loadTableConfigsObject(obj) {
    if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
      throw new TypeError('Config must be a non-array object');
    }
    ScyllaDb.#tableConfigs = obj;
    console.log('Table configs loaded from object', { count: Object.keys(obj).length });
  }
  ...
  /**
 * Extracts PK and SK names from a KeySchema definition.
 * @param {Object} tableSchema - The table schema containing KeySchema.
 * @returns {Object} - { TableName, PK, SK (optional) }
 */
  static formatSchemaFromConfig(tableSchema) {
    const keyMap = {};

    for (const entry of tableSchema.KeySchema) {
      if (entry.KeyType === 'HASH') keyMap.PK = entry.AttributeName;
      if (entry.KeyType === 'RANGE') keyMap.SK = entry.AttributeName;
    }
  ...
  static async putItem(table, item, options = {}, trackChange = false) {
    ...
    let cfg = ScyllaDb.getSchemaFromConfig(table);

    if(!cfg.PK) {
      cfg = ScyllaDb.formatSchemaFromConfig(cfg);
    }
  ...
  static async query(table, keyConditionExpr, exprVals, options = {}) {
    ...
    let payload = {
      ...
      ...options, // 👈 this now includes ExpressionAttributeNames
    };
